...
if(isset($_POST['submit']) && $_POST['submit'] == 'Сохранить') {
        ...
        edit_supplier_debt($connect, $id, $prepayment_date, $prepayment_sum, $payment_type);
}
...
<div class="page_name">
    <div class="container-fluid">
        ...
        <span class="right_cus">Редактировать оплата №<?php echo $id; ?></span>
        ...
        <form action="" method="POST" class="horizntal-form" id="input_form">
            ...
            <div class="col-md-3">
                    <select required name="payment_type" class="normalize" form="input_form">
                        <option value="<?php echo $res['payment_type']?>"><?php echo $res['payment_type']?></option>
                        <option value="Перечисление">Перечисление</option>
                        <option value="Наличные деньги">Наличные деньги</option>
                    </select>
                </div>
                <div class="col-md-3">
                    <input required value='<?php echo number_format($res['debt'], 0, ',', ' '); ?>' class="form-control autonumeric" name="prepayment_sum" form="input_form">
                <td></td>
