...
if (isset($_GET['del']) && $_GET['del'] == 'ok') {
    ...
    $sum_count = sum_count_return($connect, $id);


    if (del_return_item_tbl($connect, $pi)) {
        ...
        if (upd_return_sum($connect, $orid, $sum)) {
            if (upd_return_debt_sum($connect, $orid, $sum)) {
                header("Location: edit_inside_return.php?id=".$orid."&&payment_type=".$payment_type."&&sale_agent=".$sale_agent."&&contractor=".$contractor."&&date=".$ord_date."");   
            }
        }
...
if(isset($_POST['submit']) && $_POST['submit'] == 'Сохранить') {
    ...
    if (upd_return_item($connect, $orid, $pi, $p_name, $c_name, $pr_name, $s_name, $t_name)) {
